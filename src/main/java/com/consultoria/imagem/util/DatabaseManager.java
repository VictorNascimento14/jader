...
public class DatabaseManager {
    ...
    private static void createTables(Connection conn) throws SQLException {
        ...
        String sqlListaPecas = "CREATE TABLE IF NOT EXISTS lista_pecas (\n" +
                               ...
                               "    FOREIGN KEY (lista_id) REFERENCES listas_de_compras(id),\n" +
    ...
    public static void addListaDeCompras(ListaDeCompras lista) {
        String sql = "INSERT INTO listas_de_compras(cliente_id, dataCriacao) VALUES(?,?)";
        try (Connection conn = DriverManager.getConnection(DB_URL);
             PreparedStatement pstmt = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            pstmt.setInt(1, lista.getCliente().getId());
            pstmt.setString(2, lista.getDataCriacao().toString());
            pstmt.executeUpdate();
    ...
    public static List<ListaDeCompras> getAllListasDeCompras() {
        ...
        String sql = "SELECT l.id as lista_id, l.dataCriacao, c.id as cliente_id, c.nome as cliente_nome, c.contato as cliente_contato, c.observacoes as cliente_observacoes FROM listas_de_compras l JOIN clientes c ON l.cliente_id = c.id";
        try (Connection conn = DriverManager.getConnection(DB_URL);
             Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery(sql)) {
            while (rs.next()) {
    ...
    public static void updateListaDeCompras(ListaDeCompras lista) {
        String sql = "UPDATE listas_de_compras SET cliente_id = ?, dataCriacao = ? WHERE id = ?";
        try (Connection conn = DriverManager.getConnection(DB_URL);
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setInt(1, lista.getCliente().getId());
            pstmt.setString(2, lista.getDataCriacao().toString());
            ...
            // Clear existing pecas and add new ones
            clearPecasFromLista(lista.getId());
            for (Peca peca : lista.getPecas()) {
                addPecaToLista(lista.getId(), peca.getId());
            }
    ...
    public static void deleteListaDeCompras(int id) {
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL);
            ...
            pstmt.setInt(1, id);
            pstmt.executeUpdate();
            clearPecasFromLista(id); // Also delete entries in lista_pecas
        } catch (SQLException e) {
            System.out.println(e.getMessage());
        }
    }
    ...
    private static void clearPecasFromLista(int listaId) {
    ...
    public static List<Peca> getPecasByListaId(int listaId) {
        List<Peca> pecas = new ArrayList<>();
        String sql = "SELECT p.id, p.nome, p.descricao, p.fotoPath, p.loja_id, p.valor, p.observacoes, l.nome as loja_nome, l.endereco as loja_endereco, l.contato as loja_contato FROM pecas p JOIN lojas l ON p.loja_id = l.id JOIN lista_pecas lp ON p.id = lp.peca_id WHERE lp.lista_id = ?";
        try (Connection conn = DriverManager.getConnection(DB_URL);
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setInt(1, listaId);
            ResultSet rs = pstmt.executeQuery();
